...
For more information and release notes regarding application specific handling,  refer to:

+ Apple's Contacts forms - See the  [documentation](clasew-contacts.md)
+ Microsoft Outlook forms - See the  [documentation](clasew-outlook.md)
...
<tr><td>keywords</td><td>(required) String specifying the target workbook to operate on. Must include the file name extension</td><td>"clasew-ex4.xlsx",<br>"My Workbook.numbers"</td></tr>
<tr><td>vectors</td><td>(required) A flag indicating that if the workbook identified in wkbk is not loaded in Excel already, that it should or should not be created</td><td>(predefined vars)<br>create,<br>no-create</td></tr>
<tr><td>maps</td><td>(required) A flag indicating that, if the workbook identified in wkbk is not loaded in Excel already, an attempt should be or should not be made to open it</td><td>(predefined vars)<br>open,<br>no-open</td></tr>
</table>
